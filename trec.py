...
class TrecEvaluation:
    ...
    def __init__(self, queries, qrels):
        ...
        types_dict = {'query_id': str, 'dummy': str, 'docid': str, 'rel':int}
        self.relevance_judgments = pd.read_csv(qrels, sep='\t', names=['query_id', 'dummy', 'docid', 'rel'], dtype=types_dict)
        ...
        self.rel_per_query = {}
        for qid in np.unique(self.relevance_judgments['query_id']):
            aux = self.relevance_judgments.loc[self.relevance_judgments['query_id'] == qid]
            rel_docs = aux.loc[aux['rel'] != 0]
            self.rel_per_query[qid] = rel_docs


    def fast_p10(self, rank, query_id):

        result = rank.head(10)
        total_retrieved_docs = result.count()[0]
        rel_docs = self.rel_per_query[query_id]
    ...
    def eval(self, result, query_id):
        ...
        total_true_pos = np.intersect1d(result['_id'], query_rel_docs)
        recall = np.size(total_true_pos) / total_relevant
    ...
    def evalPR(self, scores, query_id):
        ...
        idx_rel_docs = self.rel_per_query[query_id]
